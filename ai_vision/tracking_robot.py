...
PUNCH_REACH_RATIO = 0.50         # 주먹질 오탐 방지를 위해 기준을 바운딩 박스 너비의 50%로 상향
...
REID_SIMILARITY_THRESH = 0.50   # 🌟 [수정됨] 자세 변화에 따른 조도 차이를 극복하기 위해 재식별 허용 임계값을 50%로 하향
KP_CONF_THRESH = 0.6            
MAX_LOST_FRAMES_BUFFER = 30     # 🌟 [수정됨] 라즈베리파이의 낮은 FPS를 고려하여 관성 유지 버퍼를 30프레임(약 3초)으로 대폭 확장
...
def remap_coco_to_custom(coco_kpts):
    custom_kpts = [[0.0, 0.0, 0.0] for _ in range(11)]
    if len(coco_kpts) < 17: return custom_kpts
    
    custom_kpts[KP_FACE]       = coco_kpts[0]   
    custom_kpts[KP_R_FOOT]     = coco_kpts[16]  
    custom_kpts[KP_L_FOOT]     = coco_kpts[15]  
    custom_kpts[KP_R_SHOULDER] = coco_kpts[6]
    custom_kpts[KP_R_ELBOW]    = coco_kpts[8]
    custom_kpts[KP_R_WRIST]    = coco_kpts[10]
    custom_kpts[KP_L_SHOULDER] = coco_kpts[5]
    custom_kpts[KP_L_ELBOW]    = coco_kpts[7]
    custom_kpts[KP_L_WRIST]    = coco_kpts[9]

    if coco_kpts[5][2] > KP_CONF_THRESH and coco_kpts[6][2] > KP_CONF_THRESH:
        custom_kpts[KP_CHEST] = [(coco_kpts[5][0] + coco_kpts[6][0])/2, (coco_kpts[5][1] + coco_kpts[6][1])/2, min(coco_kpts[5][2], coco_kpts[6][2])]
    if coco_kpts[11][2] > KP_CONF_THRESH and coco_kpts[12][2] > KP_CONF_THRESH:
        custom_kpts[KP_HIP] = [(coco_kpts[11][0] + coco_kpts[12][0])/2, (coco_kpts[11][1] + coco_kpts[12][1])/2, min(coco_kpts[11][2], coco_kpts[12][2])]
    return custom_kpts

def get_color_hist(frame, bx, by, bw, bh):
    """ 
    🌟 [수정됨] 객체 Bounding Box의 중앙 50% 영역(몸통 위주)만 크롭하여 히스토그램 추출.
    배경(바닥, 벽) 노이즈를 제거하여 앉거나 일어설 때 색상 정보가 오염되는 현상을 완벽히 차단함.
    """
...
def analyze_behavior(kpts, bw, bh, weapon_centers):
    def is_valid(idx): return kpts[idx][2] > KP_CONF_THRESH
    ...
    if is_valid(KP_HIP) and bh > 0:
        for f_idx in [KP_R_FOOT, KP_L_FOOT]:
            if is_valid(f_idx):
                # 🌟 [수정됨] 앉은 자세 오탐 원천 차단: 비율을 버리고, 발(Y)이 골반(Y)보다 무조건 높이 들려야만 킥으로 판정
...
class InferenceEngine:
    ...
    def run(self):
        ...
        while not self.stopped:
            ...
            try:
                # 🌟 [수정됨] conf=0.25 로 설정하여 자세 변화에 따른 Confidence 드랍을 수용하고 끈질기게 추적
                results = self.model.track(frame, tracker="bytetrack.yaml", persist=True, verbose=False, classes=[0, 41, 67], imgsz=320, conf=0.25)
                ...
                if results and results[0].boxes:
                    for i, box_obj in enumerate(results[0].boxes):
                        ...
                        if cls == 67: weapon_centers.append((bx, by)); continue
                        if cls == 41: 
